"""
pipeline_dev.py — SIOP 2026 correlation extraction (development / trust × SWB corpus)
====================================================================================

Purpose
-------
This file extracts zero-order bivariate Pearson correlations between trust-related
predictors and subjective well-being outcomes from primary-study PDFs for the SIOP
2026 machine-learning meta-analysis competition. It is the development (127-study)
pipeline: fixed trust × SWB construct definitions, competition MSE 0.013641 on the
dev leaderboard (trust × wellbeing operationalization).
...
USAGE:
  ...
  python pipeline_dev.py single pdfs/study1.pdf --study-id study1 --model phi4 2>$null
  # JSON only (e.g. PowerShell ConvertFrom-Json): add --json-summary

  # Full batch:
  python pipeline_dev.py batch --pdf-dir pdfs/ --articles-csv data/dev_articles.csv \
      --output-csv submission_dev.csv --log-json pipeline_log_dev.json --model phi4
  # Subset of studies: add --study-filter study2,study50

  # Check PDF availability:
  python pipeline_dev.py check --pdf-dir pdfs/

  # Opus API sweep (after vision batch; needs ANTHROPIC_API_KEY):
  python pipeline_dev.py opus-sweep --log-json pipeline_log_v11_final.json \
...
import json, logging, math, re, os, csv, time, argparse, sys, statistics, glob, tempfile, threading
...
# LANDMINE: Sign-flip asymmetry (see KNOWN LANDMINES #3). Do not treat “negative affect”
# as automatic flip without checking inverse-keyed *predictor* wording via this helper.

def is_distrust_predictor(label: str) -> bool:
    """Return True if predictor is reverse-keyed (higher = less trust)."""
    ll = label.lower()
    return any(d in ll for d in DISTRUST_LABELS)
...
# LANDMINE: Single choke-point for trust vs wellbeing vs exclude. Every tier must route
# labels here; diverging heuristics elsewhere desync validation from extraction.
...
def is_negative_outcome(label: str) -> bool:
    return any(n in label.lower() for n in NEGATIVE_TERMS)
...
# ═══════════════════════════════════════════════════════════════════════════
#
# WHEN THIS STAGE RUNS: after any tier produced a multiset of trust×wellbeing rows.
# WHAT IT DOES:        Resolves duplicate outcomes, wave policy, life-sat priority, etc.,
#                      before arithmetic averaging inside process_study.
...
def _filter_effects_for_meta_aggregate_trust_wellbeing(effects: list) -> list:
    ...
    WHAT: Returns list[dict] subset of input effects chosen for aggregate_r averaging.
    ASSUMES: Effects already passed basic validate_effect where applicable; uses classify_var.
    """
    if not effects or len(effects) <= 1:
        return effects
...
def extract_via_pdfplumber(pdf_path: str, verify_trust_items: bool = True) -> list:
    ...
    WHEN: First structured pass inside process_study / extract_aggregate_effect_size
        whenever PDFPLUMBER_AVAILABLE (before Docling / vision / regex).
    WHAT: Returns list[dict] partial effect rows (predictor_measure, outcome_measure,
        stat_value, source='pdfplumber', …); trust×SWB gating happens downstream.
    ASSUMES: pdf_path readable by pdfplumber; verify_trust_items usually False in
        process_study (item verification applied once on merged structured batch).
    """
    ...
    for tr in trust_rows:
        ...
        for wr in wellbeing_rows:
            ...
            for g in range(subgroup_count):
                ...
                pred = row_labels[tr]
                outc = row_labels[wr]
                flip = is_negative_outcome(outc) ^ is_distrust_predictor(pred)
...
def extract_via_docling(pdf_path: str, verify_trust_items: bool = True) -> list:
    ...
    WHEN: process_study / extract_aggregate_effect_size after pdfplumber (+ optional MinerU)
        when DOCLING_AVAILABLE and earlier tiers returned empty.
    WHAT: Returns list[dict] candidate effects (source often 'docling'); bbox metadata
        enables cross_validate_with_vision on crops.
    ...
    try:
        import gc
        # Disable OCR for speed — academic PDFs are text-based
        # OCR (RapidOCR) adds 45-60s per study with no accuracy benefit
        try:
            from docling.datamodel.pipeline_options import PdfPipelineOptions
            pipeline_options = PdfPipelineOptions()
            # Conditional OCR: enable only for PDFs with sparse text (likely scanned)
            # Born-digital PDFs (chars/page > 500) are harmed by OCR —
            # re-rasterization introduces recognition errors on numbers, Greek
            # letters, and minus signs (Hunter & Schmidt, 2004, Ch. 12 notes
            # that transcriptional errors are uncorrectable in meta-analysis)
            try:
                import fitz as _fitz
                _doc = _fitz.open(pdf_path)
                _chars = sum(len(p.get_text()) for p in _doc) / max(len(_doc), 1)
                _doc.close()
                _needs_ocr = _chars < 250  # threshold: scanned page typically <100 chars
            except Exception:
                _needs_ocr = False  # default to no OCR if check fails
            ...
            # Fallback to default if options API differs
            converter = DoclingConverter()
...
def extract_stat_candidates(content):
    ...
    apa_table = extract_apa_correlation_table(full_text)
    candidates = [_enrich_candidate_context(c, full_text) for c in by_key.values()]
    if apa_table and not candidates:
        # Only use this pass if regex found nothing — avoids duplication
        candidates.append({
            "stat_type":  "apa_table",
            "stat_value": None,
            "n":          None,
            "df":         None,
            "context":    apa_table[:800],
    ...
    # Sort: named stats first (more reliable), then table values, then by |r|
    candidates.sort(key=lambda x: (
        0 if x["source"] == "named_stat" else 1,
        0 if x["source"] == "apa_table" else 1,
        -float(x.get("attribution_score", 0.5)),
        -abs(x["stat_value"]) if x["stat_value"] else 0
    ))
...
def extract_apa_correlation_table(full_text):
    """
    Detect and extract APA-format correlation matrices where values appear
    as bare numbers in numbered columns (no r= prefix).
    Returns the raw table text if found, else None.
    """
    text_lower = full_text.lower()

    # Look for APA correlation matrix markers
    markers = [
        "descriptive statistics and", "intercorrelations",
        "correlations among", "means, standard deviations",
        "m sd 1", "m  sd  1", "mean sd 1",
    ]
    ...
    for marker in markers:
        ...
        # Extract up to 1500 chars from this point
        table_chunk = full_text[max(0, idx-50):idx+1500]

        # Verify it looks like a correlation matrix:
        # Should have numbers in (.xx) format on diagonal (reliabilities)
        # and decimal values in columns
        has_diagonal = bool(re.search(r'\(\.\d{2,3}\)', table_chunk))
        has_decimals  = len(re.findall(r'-?\.\d{2,3}', table_chunk)) > 3

        if has_diagonal or has_decimals:
            return table_chunk.strip()
...
def format_candidates_for_llm(candidates):
    ...
    for i, c in enumerate(candidates[:20]):  # hard cap at 20
        if c["source"] == "apa_table":
            # Truncate table text to keep prompt manageable
            table_text = c["context"][:600]
            lines.append(
...
def _label_grounded_in_context(label: str, context: str, global_context: str = "") -> bool:
    ...
    if ("," in lbl or len(content_toks) >= 4) and not (lbl in ctx or lbl in gctx):
        return False
...
EXCLUDE outcomes measuring importance/value/attitude rather than experience:
  e.g. "importance of close relationships", "value of family", "religious importance"
...
EXCLUDE: measures of what the participant VALUES, believes, or considers important
  ("How important are close relationships to you?" → attitude, not experience)
...
def classify_candidates(candidates, research_question, predictor, outcome, model):
    """
    Tier 2b: phi4 (or `model`) labels regex candidates as admissible trust×SWB Pearson r rows.

    WHEN: After extract_stat_candidates whenever Tier 2 runs.
    WHAT: Returns list[dict] validated effects ready for validate_effect / aggregation.
    ASSUMES: candidates carry 'context'; research_question/predictor/outcome strings match batch CSV.
    """
    if not candidates:
        return []

    # Pre-filter: remove candidates that are almost certainly not correlations
    FIGURE_CAPTION_SIGNALS = [
        "cross-lagged", "path coefficient", "cfi", "rmsea", "comparative fit",
        "figure", "controlling for age", "controlling for gender",
        "w2(", "chi-square", "latent growth", "lgm",
        # Regression table signals — values from multivariate models, not bivariate r
        "odds ratio", "95% confidence interval", "confidence interval",
        "female gender", "age (in years)", "education", "income",
        "year 0.", "year <0.",   # time trend coefficients
        "<0.01", "<#0.01",       # very small regression coefficients
        "b, 95%", "b [",        # unstandardized regression output
    ]
    filtered = []
    for c in candidates:
        ctx_lower = c["context"].lower()
        if any(sig in ctx_lower for sig in FIGURE_CAPTION_SIGNALS):
            continue  # skip figure caption values
        filtered.append(c)

    if not filtered:
        return []  # all candidates were figure caption artifacts
    ...
    for attempt in range(2):  # max 2 attempts
        try:
            ...
            for eff in effects:
                idx = eff.get("candidate_index", 0) - 1
                if 0 <= idx < len(candidates):
                    cand = candidates[idx]
                    # LLM can't change the stat_value — use the regex-extracted one
                    eff["stat_value"] = cand["stat_value"]
                    eff["stat_type"]  = cand["stat_type"]
...
def apply_direction(r, eff):
    ...
    if needs_flip:
        ...
        r = abs(r)
    # Only apply direction_positive flip when NOT needs_sign_flip
    # to avoid double-flip
    if not needs_flip and eff.get("direction_positive", True) is False:
        r = -r
    return r
...
def is_adjusted_beta(eff):
    ...
    """
    notes = (eff.get("notes") or "").lower()
    signals = ["controlling for", "adjusted for", "covariate", "after accounting",
               "model 2", "model 3", "fixed effect", "propensity", "multilevel",
               "hierarchical regression", "step 2", "block 2"]
...
def validate_effect(eff):
    ...
    if not eff.get("is_bivariate", True):
    ...
    # Rule 5: Plausibility checks for trust × wellbeing pairs
    r_conv = eff.get("r_converted") or 0
    ...
    # Require one variable to be trust and the other to be wellbeing
    # Prevents wellbeing×wellbeing and trust×trust pairs from leaking through
    ...
    # Reject same-construct reliability correlations
...
def extract_aggregate_effect_size(
    ...
    if structured_effects:
        ...
        if verify_trust_items:
            ...
            if _sk:
                result["skipped_effects"].extend(_sk)
    ...
    effects = classify_candidates(
        candidates, research_question, predictor_description, outcome_description, model
    )

    # Convert and aggregate
    r_values = []
    for eff in effects:
        ok, _ = validate_effect(eff)
        if not ok:
            continue
        r_raw, _ = convert_to_r(eff)
        if r_raw is None:
            continue
...
MANUAL_OVERRIDES = {
    ...
    "study47": {
        "r": None,
        "note": "Confirmed blank: parent-reported trust and child symptoms — not same-respondent self-report.",
        "source": "SIOP benchmark / image-verified",
        "effects": [],
    },
...
def write_supplement_review_report_file(all_results: dict | None = None) -> None:
    ...
    lines.append("Generated by pipeline_dev.py batch report")
    lines.append("=" * 72)
    lines.append("")
...
def _pack_validated_supplement_effects(structured_effects: list, result: dict, src: str):
    ...
    for eff in structured_effects:
        ...
        ok, reason = validate_effect(eff)
        if not ok:
            result["skipped_effects"].append({"label": label, "reason": f"supplement: {reason}"})
            continue
        pred_lbl = eff.get("predictor_measure", "") or ""
        outc_lbl = eff.get("outcome_measure", "") or ""
        should_flip = is_negative_outcome(outc_lbl) ^ is_distrust_predictor(pred_lbl)
...
def _try_supplement_extraction(result: dict, study_id: str, pdf_path: str) -> None:
    ...
    if not r_vals:
        ...
        sk_txt = "; ".join(
            f"{s.get('label', '?')}: {s.get('reason', '')}" for s in sk[-10:]
        )
        _register_supplement_review_event(
            4,
            study_id,
...
def process_study(pdf_path, study_id, model=OLLAMA_MODEL,
    ...
    if study_id in MANUAL_OVERRIDES:
        ...
        if corr_override:
            ...
            if _candidates:
                ...
                _effects = classify_candidates(
                    _candidates, research_question, predictor, outcome, model
                )
                _r_vals = []
                for _e in _effects:
    ...
    if verify_trust_items and structured_effects:
        ...
        if _trust_skipped:
            result["skipped_effects"].extend(_trust_skipped)
    ...
    if structured_effects:
        ...
        for eff in structured_effects:
            ...
            if pred_cls == "trust" and outc_cls == "wellbeing":
                ...
                if len(candidates) > 1:
                    ...
                    if vspan > DUPLICATE_PAIR_R_SPAN:
                        result["notes"].append(
        ...
        for eff in structured_effects:
            ...
            label = f"{eff.get('predictor_measure','?')} x {eff.get('outcome_measure','?')}"
            if not ok:
                result["skipped_effects"].append({"label": label, "reason": reason})
            ...
            # encodes the asymmetry; see KNOWN_LANDMINES in module docstring.
            should_flip = is_negative_outcome(outc_lbl) ^ is_distrust_predictor(pred_lbl)
            raw         = float(eff.get("stat_value") or 0)
        ...
        for _page_idx in _cand_pages:
            try:
                ...
                for (_rl, _cl), _val in _mat.items():
                    ...
                    if _pred_cls != "trust" or _outc_cls != "wellbeing":
                        continue
                    _flip = is_negative_outcome(_cl) ^ is_distrust_predictor(_rl)
                ...
                for _page_nums in [
                    ...
                    try:
                        ...
                        for (_rl, _cl), _val in _mat2.items():
                            ...
                            if _pred_cls != "trust" or _outc_cls != "wellbeing":
                                continue
                            _flip = is_negative_outcome(_cl2) ^ is_distrust_predictor(_rl2)
    ...
    if text_matrix_effects:
        text_matrix_effects = _filter_effects_for_meta_aggregate_trust_wellbeing(text_matrix_effects)
    ...
    if vision_effects:
        ...
        for eff in vision_effects:
            ...
            if any(s in src_l or s in nts_l for s in [
                ...
                result["skipped_effects"].append({
                    "label":  f"{pred} x {outc}",
                    "reason": "vision: figure/model-derived value — not bivariate Pearson r"
                })
                continue
            n_val = eff.get("n")
            try:
                if n_val is not None and int(n_val) >= 10000 and "vision" in src_l:
                    result["skipped_effects"].append({
                        "label":  f"{pred} x {outc}",
                        "reason": f"vision: very large pooled N ({n_val}) — likely aggregate/model result"
                    })
                    continue
            except Exception:
                pass
            ...
            if pred_cls != "trust" or outc_cls != "wellbeing":
                result["skipped_effects"].append({
                    "label":  f"{pred} x {outc}",
                    "reason": f"vision: pred={pred_cls} outc={outc_cls} — not trust×wellbeing"
                })
                continue
            ok, reason = validate_effect(eff)
            if not ok:
                result["skipped_effects"].append({"label": f"{pred} x {outc}", "reason": reason})
                _record_cross_wave_exclusion(result, eff, reason)
                continue
        ...
        if r_values:
            ...
            valid_effs = _dedupe_vision_trust_wellbeing_effects(
                _wave_stratum_then_mean_merge(valid_effs)
            )
    ...
    if remaining_candidates:
        for run_idx in range(N_RUNS):
            run_effects = classify_candidates(
                remaining_candidates, research_question, predictor, outcome, model
            )
    ...
    for eff in effects:
        ...
        if not (_label_grounded_in_context(eff.get("predictor_measure", ""), _ctx, _global_grounding_context) and
            ...
            result["skipped_effects"].append({
                "label":  label,
                "reason": "regex: measure labels not grounded in candidate context"
            })
            continue
        ...
        if _attr <= 0.20 and _looks_cited and _sec in ("introduction", "discussion", "other"):
            result["skipped_effects"].append({
                "label": label,
                "reason": f"regex: low attribution ({_attr:.2f}) + citation cues in {_sec}"
            })
            continue
        # classify_var() gate — authoritative construct filter, same as vision tier
        # phi4 labels are advisory; Python term lists are the source of truth
        pred_cls = classify_var(eff.get("predictor_measure", "") or "")
        outc_cls  = classify_var(eff.get("outcome_measure",  "") or "")
        if pred_cls != "trust" or outc_cls != "wellbeing":
            result["skipped_effects"].append({
                "label":  label,
                "reason": f"regex: pred={pred_cls} outc={outc_cls} — not trust×wellbeing"
            })
            continue
        ok, reason = validate_effect(eff)
        if not ok:
        ...
        if r_raw is None:
            result["skipped_effects"].append({"label": label, "reason": conv_note})
            continue
        ...
        authoritative_flip = (is_negative_outcome(outc_name) ^
                              is_distrust_predictor(pred_name))
        eff["needs_sign_flip"] = authoritative_flip
        r_final = apply_direction(r_raw, eff)
        r_values.append(r_final)
        result["individual_effects"].append({
            "label":            label,
    ...
    if r_values:
        ...
        result["notes"].append("No effects survived validation/conversion")
...
def run_batch(pdf_dir, articles_csv, output_csv, log_json, model=OLLAMA_MODEL,
    ...
    with open(articles_csv, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f): studies.append(row)
    if study_filter_ids:
        studies = [s for s in studies if s.get("studyid") in study_filter_ids]
...
def run_check(pdf_dir, articles_csv):
    ...
    with open(articles_csv, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f): studies.append(row)
    present = [s for s in studies if os.path.exists(os.path.join(pdf_dir, f"{s['studyid']}.pdf"))]
    missing = [s for s in studies if not os.path.exists(os.path.join(pdf_dir, f"{s['studyid']}.pdf"))]
    print(f"PDFs found: {len(present)}/{len(studies)}")
    if missing:
        for s in missing: print(f"  missing: {s['studyid']}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="pipeline_dev.py",
        description="Open-source meta-analysis pipeline v11 (two-stage, no hallucination)",
    )
    ...
    p.add_argument("--output-csv",   default="submission_dev.csv")
    p.add_argument("--log-json",     default="pipeline_log_dev.json")
